...
Controllable::Controllable(const Type &type, const String &niceName, const String &description, bool enabled) : ScriptTarget("", this, "Controllable"),
																												type(type),
																												niceName(""),
																												shortName(""),
...
{
	if (niceName == _niceName)
		return;

	this->niceName = _niceName;
	if (!hasCustomShortName)
		setAutoShortName();
	else
	{
...
{
	if (isControllableFeedbackOnly == value)
		return;
...
void Controllable::setParentContainer(ControllableContainer *container)
{
	...
	if (parentContainer != nullptr)
		parentContainer->removeControllable(this, false);
...
{
	if (hideInRemoteControl != defaultHideInRemoteControl)
		return true;
	if (userCanSetReadOnly && isControllableFeedbackOnly)
		return true;
	if (!isSavable)
		return false;
	if (!saveValueOnly)
		return true;
	if (saveCustomData && !customData.isVoid())
		return true;
	return false;
}
...
var Controllable::getJSONData(ControllableContainer *relativeTo)
{
	...
	if (canBeDisabledByUser)
		data.getDynamicObject()->setProperty("enabled", enabled);
	if (hideInRemoteControl != defaultHideInRemoteControl)
		data.getDynamicObject()->setProperty("hideInRemoteControl", hideInRemoteControl);
	if (userCanSetReadOnly)
		data.getDynamicObject()->setProperty("feedbackOnly", isControllableFeedbackOnly);
	if (saveCustomData && !customData.isVoid())
		data.getDynamicObject()->setProperty("customData", customData);
	...
	if (hasCustomShortName)
		data.getDynamicObject()->setProperty("shortName", shortName);
...
void Controllable::loadJSONData(var data)
{
	...
	if (data.getDynamicObject()->hasProperty("type"))
		saveValueOnly = false;
	if (data.getDynamicObject()->hasProperty("niceName"))
		setNiceName(data.getProperty("niceName", ""));
	if (data.getDynamicObject()->hasProperty("shortName"))
		setCustomShortName(data.getProperty("shortName", ""));
	if (data.getDynamicObject()->hasProperty("enabled"))
		setEnabled(data.getProperty("enabled", enabled));
	...
	if (data.getDynamicObject()->hasProperty("feedbackOnly"))
		setControllableFeedbackOnly(data.getProperty("feedbackOnly", false));
...
{
	if (data.hasProperty("shortName"))
		setCustomShortName(data.getProperty("shortName", "[error]").toString());
	if (data.hasProperty("description"))
		description = data.getProperty("description", "[no description]").toString();
	if (data.hasProperty("readOnly"))
		setControllableFeedbackOnly(data.getProperty("readOnly", false));
	else if (data.hasProperty("feedbackOnly"))
		setControllableFeedbackOnly(data.getProperty("feedbackOnly", false));

	StringArray attributes = getValidAttributes();
	for (auto &a : attributes)
	{
		if (data.hasProperty(a))
			setAttribute(a, data.getDynamicObject()->getProperty(a));
	}
}
...
{
	if (dashboardDefaultLabelParentLevel == 0)
		return niceName;
	String s = "";
	...
	{
		if (cParent != nullptr)
			cParent = cParent->parentContainer;
	}
	if (cParent != nullptr)
		s = cParent->niceName;
	if (dashboardDefaultAppendLabel)
		s += " : " + niceName;
...
bool Controllable::setAttributeInternal(String param, var value)
{
	if (param == "description")
		description = value;
	else if (param == "readonly" || param == "readOnly")
		setControllableFeedbackOnly(value);
	else if (param == "enabled")
		setEnabled(value);
	else if (param == "canBeDisabled")
		canBeDisabledByUser = (bool)(int)value;
	else if (param == "saveValueOnly")
		saveValueOnly = value;
	else
		return false;
		...
		else
		{
			...
			case Controllable::Type::BOOL:
				...
				if (valueIsABool)
					...
					((BoolParameter *)c)->setValue((float)value > .5);
				break;

			case Controllable::Type::FLOAT:
				if (valueIsABool || valueIsANumber)
			...
			case Controllable::Type::COLOR:
				...
				{
					((ColorParameter *)c)->setColor(Colour::fromFloatRGBA((float)(value[0]), (float)(value[1]), (float)(value[2]), value.size() > 3 ? (float)(value[3]) : 1));
				}
				else if (a.numArguments >= 3)
				{
					((ColorParameter *)c)->setColor(Colour::fromFloatRGBA((float)(a.arguments[0]), (float)(a.arguments[1]), (float)(a.arguments[2]), a.numArguments > 3 ? (float)(a.arguments[3]) : 1));
				}
				...
				break;
			case Controllable::Type::INTRANGE:
				if (value.isArray() && value.size() >= 2)
					((IntRangeParameter *)c)->setPoint((int)value[0], (int)value[1]);
				else if (a.numArguments >= 2)
					((IntRangeParameter *)c)->setPoint((int)a.arguments[0], (int)a.arguments[1]);
				break;
			case Controllable::Type::POINT2D:
				if (value.isArray() && value.size() >= 2)
					((Point2DParameter *)c)->setPoint((float)value[0], (float)value[1]);
				else if (a.numArguments >= 2)
					((Point2DParameter *)c)->setPoint((float)a.arguments[0], (float)a.arguments[1]);
				break;

			case Controllable::Type::POINT3D:
				if (value.isArray() && value.size() >= 3)
					((Point3DParameter *)c)->setVector((float)value[0], (float)value[1], (float)value[2]);
				else if (a.numArguments >= 3)
					((Point3DParameter *)c)->setVector((float)a.arguments[0], (float)a.arguments[1], (float)a.arguments[2]);
				break;
			...
			case Controllable::Type::TARGET:
				if (value.isObject())
				{
					...
					if (tp->targetType == TargetParameter::CONTROLLABLE)
					{
						...
						if (target != nullptr)
							...
							LOGWARNING("Set target from script, Target not found");
					}
					else
					{
				...
				else if (value.isString())
				{
					...
					if (tp->targetType == TargetParameter::CONTROLLABLE)
					{
						...
						if (target != nullptr)
							tp->setValueFromTarget(target);
						else
							LOGWARNING("Set target from script, Target not found : " << value.toString());
					}
					else
					{
...
var Controllable::setNameFromScript(const juce::var::NativeFunctionArgs &a)
{
	...
	if (a.numArguments >= 2)
		c->setCustomShortName(a.arguments[1].toString());
	else
		c->setAutoShortName();
...
{
	if (controllableRef != nullptr && !controllableRef.wasObjectDeleted())
		return controllableRef.get();
