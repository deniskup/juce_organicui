...
IntRangeUI::IntRangeUI(Array<IntRangeParameter*> parameters) :
	ParameterUI(Inspectable::getArrayAs<IntRangeParameter, Parameter>(parameters)),
	p2ds(parameters),
	p2d(parameters[0]),
...

{
	...
	//int parameters don't have a default UI for some reason
	...
	yParam.addAsyncParameterListener(this);
	 ...
	 xSlider.reset((ParameterUI*)xParam.createStepper());
	 ySlider.reset((ParameterUI*)yParam.createStepper());
...
void IntRangeUI::mouseDownInternal(const MouseEvent& e)
{
...
void IntRangeUI::mouseUpInternal(const MouseEvent&)
{
...
void IntRangeUI::paint(Graphics& g)
{
	ParameterUI::paint(g);
...
void IntRangeUI::showEditWindowInternal()
{
...
void IntRangeUI::showEditRangeWindowInternal()
{
...
void IntRangeUI::updateUseExtendedEditor()
{
	float baseHeight = GlobalSettings::getInstance()->fontSize->floatValue() + 4;
	...
	if (p2d->showExtendedEditor && canShowExtendedEditor)
	{
		if (canvasUI == nullptr)
		{
			//commented because P2DUI cannot be used with IntRangeParameter
...
void IntRangeUI::rangeChanged(Parameter* p)
{
...
void IntRangeUI::updateUIParamsInternal()
{
...
void IntRangeUI::newMessage(const Parameter::ParameterEvent& e)
{
	...
	{
		isUpdatingFromParam = true;
		xParam.setValue(((IntRangeParameter*)e.parameter)->x);
		yParam.setValue(((IntRangeParameter*)e.parameter)->y);
		isUpdatingFromParam = false;
	}
